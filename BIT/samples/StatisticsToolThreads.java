...
{
    private static Map<Long, Integer> dyn_method_count = new HashMap<>();
    private static Map<Long, Integer> dyn_bb_count = new HashMap<>();
    private static Map<Long, Integer> dyn_instr_count = new HashMap<>();

    private static Map<Long, Integer> newcount = new HashMap<>();
    private static Map<Long, Integer> newarraycount = new HashMap<>();
    private static Map<Long, Integer> anewarraycount = new HashMap<>();
    private static Map<Long, Integer> multianewarraycount = new HashMap<>();

    private static Map<Long, Integer> loadcount = new HashMap<>();
    private static Map<Long, Integer> storecount = new HashMap<>();
    private static Map<Long, Integer> fieldloadcount = new HashMap<>();
    private static Map<Long, Integer> fieldstorecount = new HashMap<>();
    ...
    public static synchronized void printDynamic(String foo)
    {
        ...
        try {
            FileWriter myWriter = new FileWriter("/home/daniela/daniela/CNV/BIT/examples/pt_output/StatisticsToolThreads_dynamic.txt", true);

            int current_dyn_method_count = dyn_method_count.get(currentThreadId);
            int current_dyn_bb_count = dyn_bb_count.get(currentThreadId);
            int current_dyn_instr_count = dyn_instr_count.get(currentThreadId);

            // put back to zero
            dyn_method_count.put(currentThreadId, 0);
            dyn_bb_count.put(currentThreadId, 0);
            dyn_instr_count.put(currentThreadId, 0);
    ...
    public static synchronized void dynInstrCount(int incr)
    {
        ...
        Integer currentDynInstrCount = dyn_instr_count.get(currentThreadId);
        if (currentDynInstrCount == null) currentDynInstrCount = 0;
        Integer currentDynBbCount = dyn_bb_count.get(currentThreadId);
        if (currentDynBbCount == null) currentDynBbCount = 0;
    ...
    public static synchronized void dynMethodCount(int incr)
    {
        ...
        Integer currentDynMethodCount = dyn_method_count.get(currentThreadId);
        if (currentDynMethodCount == null) currentDynMethodCount = 0;
    ...
    public static synchronized void printAlloc(String s)
    {
        long currentThreadId = Thread.currentThread().getId();
        try {
            FileWriter myWriter = new FileWriter("/home/daniela/daniela/CNV/BIT/examples/pt_output/StatisticsToolThreads_alloc.txt", true);
    ...
    {
        long currentThreadId = Thread.currentThread().getId();
        Integer current_newcount = newcount.get(currentThreadId);
        if (current_newcount == null) {
            current_newcount = 0;
            newcount.put(currentThreadId, 0);
        }
        Integer current_newarraycount = newarraycount.get(currentThreadId);
        if (current_newarraycount == null) {
            current_newarraycount = 0;
            newarraycount.put(currentThreadId, 0);
        }
        Integer current_anewarraycount = anewarraycount.get(currentThreadId);
        if (current_anewarraycount == null) {
            current_anewarraycount = 0;
            anewarraycount.put(currentThreadId, 0);
        }
        Integer current_multianewarraycount = multianewarraycount.get(currentThreadId);
        if (current_multianewarraycount == null) {
            current_multianewarraycount = 0;
            multianewarraycount.put(currentThreadId, 0);
        }
    ...
    public static synchronized void LSFieldCount(int type)
    {
        ...
        Integer currentFieldLoadCount = fieldloadcount.get(currentThreadId);
        if (currentFieldLoadCount == null) currentFieldLoadCount = 0;
        Integer currentFieldStoreCount = fieldstorecount.get(currentThreadId);
        if (currentFieldStoreCount == null) currentFieldStoreCount = 0;
    ...
    public static synchronized void LSCount(int type)
    {
        ...
        Integer currentLoadCount = loadcount.get(currentThreadId);
        if (currentLoadCount == null) currentLoadCount = 0;
        Integer currentStoreCount = storecount.get(currentThreadId);
        if (currentStoreCount == null) currentStoreCount = 0;
        ...
        if (argv[0].equals("-static")) {
            ...
            try {
                ...
                if (in_dir.isDirectory() && out_dir.isDirectory()) {
                    try {
                        File myObj = new File(
                                "/home/daniela/daniela/CNV/BIT/examples/pt_output/StatisticsToolThreads_dynamic.txt");
            ...
            try {
                ...
                if (in_dir.isDirectory() && out_dir.isDirectory()) {
                    try {
                        File myObj = new File(
                                "/home/daniela/daniela/CNV/BIT/examples/pt_output/StatisticsToolThreads_alloc.txt");
                        if (myObj.createNewFile()) {
                            System.out.println("File created: " + myObj.getName());
                        } else {
                            System.out.println("File already exists.");
            ...
            try {
                ...
                if (in_dir.isDirectory() && out_dir.isDirectory()) {
                    try {
                        File myObj = new File(
                                "/home/daniela/daniela/CNV/BIT/examples/pt_output/StatisticsToolThreads_load_store.txt");
                        if (myObj.createNewFile()) {
                            System.out.println("File created: " + myObj.getName());
                        } else {
                            System.out.println("File already exists.");
            ...
            try {
                ...
                if (in_dir.isDirectory() && out_dir.isDirectory()) {
                    try {
                        File myObj = new File(
                                "/home/daniela/daniela/CNV/BIT/examples/pt_output/StatisticsToolThreads_branch.txt");
                        if (myObj.createNewFile()) {
                            System.out.println("File created: " + myObj.getName());
                        } else {
                            System.out.println("File already exists.");
